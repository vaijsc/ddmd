...
def get_split_datasets(dataset_root, dataset_name, batch_size):
    member_dataset, _, member_loader, nonmember_loader = load_member_data(dataset_root=dataset_root, dataset_name=dataset_name, batch_size=batch_size,
                                                             shuffle=False, drop_last=True, randaugment=False)
    
    dataset_size = len(member_dataset)
    split_index = int(dataset_size * 0.5)
    
    generator = torch.Generator().manual_seed(1234)
    indices = torch.randperm(dataset_size, generator=generator)
    dataset1 = Subset(member_dataset, indices[:split_index])
    dataset2 = Subset(member_dataset, indices[split_index:])
    
    dataloader1 = torch.utils.data.DataLoader(dataset1, batch_size=batch_size, shuffle=False, drop_last=True)
    dataloader2 = torch.utils.data.DataLoader(dataset2, batch_size=batch_size, shuffle=False, drop_last=True)
    
    return dataloader1, dataloader2, nonmember_loader
...
def get_intermediate_results_dual(model1, model2, FLAGS, dataloader1, dataloader2, t_sec, timestep):
    target_steps = list(range(0, t_sec, timestep))[1:]

    internal_diffusion_list = []
    internal_denoised_list = []

    # Process subset1 with model2
    for batch_idx, x in enumerate(tqdm.tqdm(dataloader1)):
        x = x[0].cuda()
        x = x * 2 - 1

        x_sec = ddim_multistep(model2, FLAGS, x, t_c=0, target_steps=target_steps)
        x_sec = x_sec['x_t_target']
    ...
    # Process subset2 with model1
    for batch_idx, x in enumerate(tqdm.tqdm(dataloader2)):
        x = x[0].cuda()
        x = x * 2 - 1

        x_sec = ddim_multistep(model1, FLAGS, x, t_c=0, target_steps=target_steps)
        x_sec = x_sec['x_t_target']
...
def secmi_attack_dual(model1, model2, FLAGS, dataset_root, timestep=10, t_sec=100, batch_size=128, dataset='cifar10'):
    subset1, subset2, nonmember_loader = get_split_datasets(dataset_root, dataset, batch_size)

    member_results = get_intermediate_results_dual(model1, model2, FLAGS, subset1, subset2, t_sec, timestep)
    nonmember_results = get_intermediate_results(model1, FLAGS, nonmember_loader, t_sec, timestep)
...
if __name__ == '__main__':
    ...
    secmi_attack_dual(model1, model2, FLAGS, dataset_root=args.dataset_root, t_sec=args.t_sec, timestep=args.k, batch_size=1024, dataset=args.dataset)
