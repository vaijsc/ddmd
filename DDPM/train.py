...
flags.DEFINE_string('resume_from_ckpt', None, help='path to the checkpoint to resume from')
...
def train():
    ...
    if FLAGS.resume_from_ckpt:
        ckpt = torch.load(FLAGS.resume_from_ckpt, map_location="cpu")
        net_model.load_state_dict(ckpt['net_model'])
        print(f'Resuming from checkpoint: {FLAGS.resume_from_ckpt}')
        skip_step = ckpt['step']
        FLAGS.total_steps = FLAGS.total_steps - skip_step
        print(f"Resuming from step: {skip_step}, Remaining training steps: {FLAGS.total_steps}")
        ...
        for step in pbar:
            ...
            if FLAGS.sample_step > 0 and step % FLAGS.sample_step == 0:
                ...
                with torch.no_grad():
                    ...
                    if FLAGS.resume_from_ckpt:
                        path = os.path.join(FLAGS.logdir, 'sample', '%d.png' % (step + skip_step))
                    else:
            ...
            if FLAGS.save_step > 0 and step > 0 and step % FLAGS.save_step == 0:
                ckpt = {
                    ...
                    'step': step + skip_step if FLAGS.resume_from_ckpt else step,
                    'x_T': x_T,
                }
                if FLAGS.resume_from_ckpt:
                    torch.save(ckpt, os.path.join(FLAGS.logdir, f'ckpt-step{step + skip_step}.pt'))
                else:
