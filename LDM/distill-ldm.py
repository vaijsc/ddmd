...
from customed_datasets import ObjectAttributeDataset, get_classnames
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--partition", type=int, default=None, help="partition of training data to use"
    )
    parser.add_argument(
...
def main(args):
    ...
    if args.resume_from_checkpoint:
        ...
        unet = UNet2DConditionModel.from_pretrained(os.path.join(args.output_dir, path), subfolder="unet", revision=args.revision)
        text_encoder = text_encoder_cls.from_pretrained(os.path.join(args.output_dir, path), subfolder="text_encoder", revision=args.revision)
        global_step = int(path.split("_")[1])

    teacher1 = UNet2DConditionModel.from_pretrained(args.unet_path1, subfolder="unet", revision=args.revision)
    teacher2 = UNet2DConditionModel.from_pretrained(args.unet_path2, subfolder="unet", revision=args.revision)

    vae.requires_grad_(False)
    teacher1.requires_grad_(False)
    teacher2.requires_grad_(False)
    ...
    train_dataset1 = ObjectAttributeDataset(
    ...
    train_dataset2 = ObjectAttributeDataset(
        instance_data_root=args.instance_data_dir,
        class_prompt=args.class_prompt,
        partition=1,
        tokenizer=tokenizer,
    ...
    temp1 = train_dataset1.prompts
    temp2 = train_dataset2.prompts
    choicelist = [x[0] for x in temp1] + [x[0] for x in temp2]
    ...
    if args.duplication in ['dup_both','dup_image']:
        raise ValueError("Duplication is not supported in distillation")
    else:
        ...
        datalooper1 = infiniteloop(train_dataloader1)
        ...
        datalooper2 = infiniteloop(train_dataloader2)
    
    # Scheduler and math around the number of training steps.
    overrode_max_train_steps = False
    num_update_steps_per_epoch = math.ceil((len(train_dataloader1) + len(train_dataloader2)) / args.gradient_accumulation_steps)
    ...
    if args.train_text_encoder:
        unet, text_encoder, optimizer, datalooper1, datalooper2, lr_scheduler = accelerator.prepare(
            unet, text_encoder, optimizer, datalooper1, datalooper2, lr_scheduler
        )
    else:
        unet, optimizer, datalooper1, datalooper2, lr_scheduler = accelerator.prepare(
            unet, optimizer, datalooper1, datalooper2, lr_scheduler
        )
    ...
    teacher1.to(accelerator.device, dtype=weight_dtype)
    teacher2.to(accelerator.device, dtype=weight_dtype)
    if not args.train_text_encoder:
    ...
    logger.info(f"  Num examples = {len(train_dataset1) + len(train_dataset2)}")
    logger.info(f"  Num batches each epoch = {len(train_dataloader1) + len(train_dataloader2)}")
    ...
    if accelerator.is_main_process:
        ...
        if args.class_prompt in ['instancelevel_blip','instancelevel_ogcap']: 
            rand_prompts = np.random.choice(choicelist,2)
            
        for count,prompt in enumerate(rand_prompts):
            save_path = os.path.join(args.output_dir,"generations", f"{global_step:04d}_img{count}.png")
            ...
            images = pipeline(prompt=prompt, height=args.resolution, width=args.resolution,
                                num_inference_steps=50, num_images_per_prompt=2, generator=generator).images
            concat_h(*images, pad=2).save(save_path)
    ...
    for step in range(args.max_train_steps):
        ...
        with accelerator.accumulate(unet):
            if step % 2 == 0:
                batch = next(datalooper1)
            else:
                batch = next(datalooper2)
            ...
            with torch.no_grad():
                if step % 2 == 0:
                    target = teacher2(noisy_latents, timesteps, encoder_hidden_states).sample
                else:
                    target = teacher1(noisy_latents, timesteps, encoder_hidden_states).sample
